...
소프트웨어 공학 미니 프로젝트용 Todo 앱 보고서.

- 배포 URL: https://todolist-two-lake-33.vercel.app/
- GitHub 저장소: https://github.com/khw04/TODO_list.git
...
특히 처음에는 프론트엔드와 백엔드가 어떤 방식으로 데이터를 주고받는지, 그리고 API 요청이 어떤 흐름으로 처리되는지를 이해하는 데 시간이 많이 걸렸다. 사용자가 프론트엔드에서 입력한 값이 백엔드로 전달되고, 백엔드가 이를 처리한 뒤 데이터베이스에 저장하고 다시 응답을 보내는 전체 통신 메커니즘을 직접 구현하면서 조금씩 구조를 이해할 수 있었다.

가장 어려웠던 문제는 MongoDB Atlas 연동 과정이었다. 초기에는 연결 문자열을 넣어도 데이터베이스가 계속 연결되지 않았고, 원인을 찾는 과정에서 시간이 많이 걸렸다. 단순히 코드 문제라고 생각했지만, 실제 원인은 MongoDB Atlas의 IP 허용 설정 문제였다.

이 문제는 Atlas의 `Network Access`에서 현재 IP 또는 `0.0.0.0/0`을 허용하도록 설정한 뒤 해결할 수 있었다. 이 과정을 통해 데이터베이스 연결 문제는 코드만이 아니라 외부 서비스 설정, 보안 정책, 네트워크 허용 여부까지 함께 확인해야 한다는 점을 배울 수 있었다.
